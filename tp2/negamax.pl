...
negamax(J,Etat,P,Pmax,[nil,H]):-
	P\=Pmax,
	ground(Etat),
	heuristique(J,Etat,H).

negamax(+J, +Etat, +P, +Pmax, [Coup,Val]):-
	successeurs(J,Etat,Etat_Suiv),
	loop_negamax(J,P,Pmax,Etat_Suiv,CoupleVal),
	meilleur(CoupleVal,[Coup,Val]).
...
| ?- statistics(cpu_time,BEFORE),final_state(Fin), heuristique1(Fin,Fin,H), statistics(cpu_time,AFTER), CPU_TIME = AFTER-BEFORE.
	*/


test_negamax( Pmax , B , CPU_TIME ) :-
	statistics(cpu_time, BEFORE ),
	main( B , _ , Pmax ),
	statistics(cpu_time, AFTER ),
